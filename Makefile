CFLAGS = -Wall -Wextra -Werror -g -I libft/include -I include -O3

LDFLAGS = -L libft

LDLIBS = -l ft

MKDIR = mkdir -p
RMDIR = rm -rf

NAME = push_swap

BIN_DIR = bin/
OBJ_DIR = obj/

MAIN_SRC = main.c
UTILS_SRC = stack_init.c stack_utils.c error_free.c sort.c push_swap_init.c average_partitioning.c push_swap.c
COMMANDS_SRC = swap.c push.c rotate.c reverse_rotate.c

MAIN_OBJ = $(addprefix obj/, $(MAIN_SRC:%.c=%.o))
UTILS_OBJ = $(addprefix obj/, $(UTILS_SRC:%.c=%.o))
COMMANDS_OBJ = $(addprefix obj/, $(COMMANDS_SRC:%.c=%.o))

INCLUDE = include/push_swap.h

LIBFT_STATIC = libft/libft.a

.PHONY : all clean fclean re

all : $(LIBFT_STATIC) $(OBJ_DIR) $(NAME)

push_swap: $(MAIN_OBJ) $(UTILS_OBJ) $(COMMANDS_OBJ) $(LIBFT_STATIC)
	$(CC) $^ $(LDFLAGS) -o $@ $(LDLIBS)

obj/%.o : src/%.c $(INCLUDE)
	$(CC) $(CFLAGS) -c $< -o $@

obj/%.o : src/commands/%.c $(INCLUDE)
	$(CC) $(CFLAGS) -c $< -o $@

$(LIBFT_STATIC) :
	$(MAKE) -C libft

$(OBJ_DIR) :
	$(MKDIR) $@	

clean :
	$(MAKE) -C libft clean
	$(RMDIR) $(OBJ_DIR)

fclean :
	$(MAKE) -C libft fclean
	$(RMDIR) $(OBJ_DIR)
	$(RM) $(NAME)

re : fclean all
